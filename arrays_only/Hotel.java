...
public class Hotel{
	...
	//loop to access menu options based on the input
		...
		case "D":
		    deleteCustomer(roomNum,customers);
		    break;
	...
	//loop to traverse through all the elements in the array
	    ...
	    System.out.println("Number of customers in room " + (i+1) + " are " + customers[i].getNoOfGuests());
	    System.out.println();
	}

    }

    public static void addCustomer(int n,Person [] customers){
	...
	try{
		...
		//condition to check whether all the rooms are empty
		    ...
		    //condition to add a customer if the room is empty
	    ...
	    //get an input again to add a customer when the initial input was incorrect
	}
    ...
    public static void deleteCustomer(int n,Person []customers){
	...
	try{
		...
		//set the name back to initialised state
